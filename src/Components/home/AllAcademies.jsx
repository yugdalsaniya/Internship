...
const AllAcademies = () => {
  ...
  // Define generateSlug function within AllAcademies
    ...
    const fetchPartners = async () => {
      try {
        ...
        const mappedData = data.map((item) => ({
          id: item._id, // Store _id for routing
        ...
        {!loading && !error && partners.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {partners.map((partner) => (
              <div
                key={partner.id} // Use id as key for better performance
              ...
              >
                ...
                <Link to={`/academy/${partner.slug}/${partner.id}`}>
